...
void Camera::Reset(){
	eyePosition[0] = 300.0f;
	eyePosition[1] = 200.0f;
	eyePosition[2] = 300.0;//0.0f;
...
// This function will be only called once to set the view window
void Camera::SetViewport()
{
    // Not sure why the window sizes are actually half of the real window sizes
    // so I use twice the scene window width and height to set the view window
...
void Camera::changeCamera(float x, float y, float z) { 
    add(eyePosition, right, x);
    add(eyePosition, forward, z);
    add(eyePosition, up, y);
    SetupCamera();
}
