...
public:
	/**
	* Constructor takes in command line arguments, a title and initial window dimensions/size.
	*/
...
protected:
    ...
    // 纯虚函数，在基类中抽象出一个方法，且该基类只做能被继承，而不能被实例化
    // 这个方法必须在派生类(derived class)中被实现，否则（不实现）将会报错；而非纯虚函数可以不实现
