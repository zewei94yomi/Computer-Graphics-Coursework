...
//     light needs to be drawn after objects are drawn, so that it can be still at the same place
    if (LightOn) {
        ...
        GLfloat position[] = {280, 200, 220, 1};
        glLightfv(GL_LIGHT0, GL_AMBIENT, ambience);
        glLightfv(GL_LIGHT0, GL_DIFFUSE, diffuse);
        glLightfv(GL_LIGHT0, GL_SPECULAR, specular);
        glLightfv(GL_LIGHT0, GL_POSITION, position);
    } else {
        GLfloat ambience[] = {0.1f, 0.1f, 0.1f, 1.0f};
        GLfloat diffuse[] = {0.15f, 0.15f, 0.15f, 1.0f};
    ...
    if (SunOn) {
        glEnable(GL_LIGHTING);
        glEnable(GL_LIGHT0);
        GLfloat ambience[] = {0.2f, 0.2f, 0.2f, 1.3f};
        GLfloat diffuse[] = {0.8f, 0.8f, 0.8f, 1.0f};
        GLfloat specular[] = {1, 1, 1, 1.0f};
        GLfloat position[] = {-500, 500, -520, 1};
        glLightfv(GL_LIGHT0, GL_AMBIENT, ambience);
        glLightfv(GL_LIGHT0, GL_DIFFUSE, diffuse);
        glLightfv(GL_LIGHT0, GL_SPECULAR, specular);
        glLightfv(GL_LIGHT0, GL_POSITION, position);
