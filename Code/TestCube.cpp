...
TestCube::TestCube() : xrot(0.0f), yrot(0.0f), zrot(0.0f), scale(0.5f){}


/// draw the triangle in the scene
void TestCube::Display()
{
    glPushMatrix();
...
void TestCube::Update(const double& deltaTime)
{
    zrot += 10.0f * static_cast<float>(deltaTime);
}

/// set scale variables based on a key press
void TestCube::HandleKey(unsigned char key, int state, int x, int y)
{
    switch (key)
    {
        case 'i':
