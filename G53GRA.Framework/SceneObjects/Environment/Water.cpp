...
Water::Water(const int& gridX, const int& gridZ)
    : xGridDims(gridX), zGridDims(gridZ), frozen(false), time(0.0)
{
//    texID = Scene::GetTexture("/Users/ryan/Downloads/g53gra_framework-demo-viewing_example/G53GRA.Framework/G53GRA.Framework/Textures/water.bmp");
        ...
        glPushMatrix();
            ...
            for (int j = 0; j < zGridDims; j++)
            {
                ...
                for (int i = 0; i < xGridDims; i++)
                {
                    ...
                    glVertex3f(x, y, z + 2.0f / (float)zGridDims);
                    ...
                    glVertex3f(x + 30.0f / (float)xGridDims, y, z + 2.0f / (float)zGridDims);
                    ...
                    glVertex3f(x + 30.0f / (float)xGridDims, y, z);

                    x += 30.0f / (float)xGridDims; // move the position of the quad on by one xgriddim step in the x axis
                }
                z += 2.0f / (float)zGridDims; // move the position of the quad on by one zgriddim step in the z axis
            }
            glEnd();
